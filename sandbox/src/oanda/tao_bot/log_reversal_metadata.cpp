...
void Oanda::TaoBot::log_reversal_metadata() {
  ...
  // TODO: Decide
  Formatted::Stream should_always_normalize_color =
      this->api_client.config.should_always_normalize ? fmt.green : fmt.red;

  const std::string should_always_normalize_text =
      this->api_client.config.should_always_normalize ? "YES" : "NO";

  std::cout << fmt.bold << fmt.yellow;
  printf("Should always normalize? ");
  std::cout << should_always_normalize_color << should_always_normalize_text
            << fmt.reset << std::endl;
