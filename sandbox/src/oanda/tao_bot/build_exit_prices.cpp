  ...
  if (this->api_client.config.should_stop_profit) {
    ...
    if (stop_profit_version_ == 0.5 &&
      ...
      const double target_price_action_delta =
          0.01 * target_price_action * day_candle_.range();

      double target_price;
      double target_delta;

      if (action == order_action_t::BUY) {
        target_price = entry_reversal_mid + target_price_action_delta;
        target_delta = target_price - execution_price;
      } else {
        target_price = entry_reversal_mid - target_price_action_delta;
        target_delta = execution_price - target_price;
      }
      ...
      stop_profit = std::max(min_stop_profit, target_delta);
    }
  }
