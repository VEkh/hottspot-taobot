...
void Alpaca::TaoBot::log_reversals() {
  ...
  std::map<double, double>::iterator it = this->reversals.highs.begin();
  ...
  if (this->open_order_ptr) {
    ...
    const std::pair<double, double> reversal =
        nearest_opening_reversal(this->open_order_ptr);

    if (action == order_action_t::BUY) {
      std::cout << fmt.bold << fmt.red;
      printf("Low: %.2f @ %s", reversal.second,
             ::utils::time_::date_string(reversal.first, "%H:%M",
                                         "America/Chicago")
                 .c_str());
    } else {
      std::cout << fmt.bold << fmt.green;
      printf("High: %.2f @ %s", reversal.second,
             ::utils::time_::date_string(reversal.first, "%H:%M",
                                         "America/Chicago")
                 .c_str());
    }
